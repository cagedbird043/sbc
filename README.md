...
| 命令 | 说明 |
|------|------|
| `sbc update` | 完整部署流程：HTTPS 下载 → 渲染 → sing-box check → 部署 → 重启 |
...
| `SBC_CONFIG_VARIANT` | 覆盖配置变体 |
| `SBC_CONFIG_URLS` | 模板下载 URL 列表（.env 中配置） |
...
| 变体 | 说明 |
|------|------|
| `fakeip-prefer-ipv4` | FakeIP + prefer_ipv4（主流方向） |
